    ...
    public StatisticsUsesResponse getStatisticsUses(){
        ...
        Optional<StatisticsUsesResponse> statistic = jdbcClient.sql("""
            SELECT COUNT(U.USER_ID) AS totalUsers,
                   COUNT(CASE WHEN TRUNC(U.LAST_USE) = TRUNC(SYSDATE) THEN 1 END) AS usersToday,
                   COUNT(CASE WHEN TRUNC(U.CREATE_DATE) = TRUNC(SYSDATE) THEN 1 END) AS downloadsToday,
